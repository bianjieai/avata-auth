...
/*
     ...
     * @param query     Query 参数，例："key1=value1&key2=value2"，需传入 json 格式
     * @param body      Body 参数，例："{\"count\": 1, \"operation_id\": \"random_string\"}"，需传入 json 格式
     * @param timestamp 当前时间戳（毫秒），例：1647751123703
     * @param apiSecret 应用方的 API Secret，例："AKIDz8krbsJ5yKBZQpn74WFkmLPc5ab"
     * @return 返回签名结果
 */
function SignRequest(path, query, body, apiSecret, timestamp) {
    var params = new Map();
    params.set("path_url", path)
    if (body) {
        JSON.parse(JSON.stringify(body), function (key, value) {
            if (key) {
                params.set("body_" + key, value);
            }
        });
    }
    if (query) {
        JSON.parse(JSON.stringify(query), function (key, value) {
            if (key) {
                params.set("query_" + key, value);
            }
        });
    }
